...
- Name: pipeline.spec.ts >> Pipeline - Kanban >> un lead aparece en su columna correcta
- Location: e2e/tests/pipeline.spec.ts:36:7
...
Locator: locator('.kanban-column').filter({ hasText: 'Nutrición' }).locator('.lead-card').filter({ hasText: 'Pipeline Test' })
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('.kanban-column').filter({ hasText: 'Nutrición' }).locator('.lead-card').filter({ hasText: 'Pipeline Test' })
...
- paragraph: Arrastra los leads entre etapas del funnel
```
