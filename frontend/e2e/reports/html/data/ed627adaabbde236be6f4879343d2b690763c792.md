...
- Name: pipeline.spec.ts >> Pipeline - Kanban >> muestra los nombres correctos de las columnas
- Location: e2e/tests/pipeline.spec.ts:21:7
...
Locator: locator('.kanban-column').filter({ hasText: 'Atracción' })
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('.kanban-column').filter({ hasText: 'Atracción' })
