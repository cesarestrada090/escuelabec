...
- Name: pipeline.spec.ts >> Pipeline - Kanban >> drag & drop mueve el lead a otra columna
- Location: e2e/tests/pipeline.spec.ts:68:7

# Error details

```
TimeoutError: locator.dragTo: Timeout 15000ms exceeded.
Call log:
  - waiting for locator('.lead-card').filter({ hasText: 'DragDrop Test' })
